...
which may cause out-of-memory errors, when the data is extreamly huge.
...
In `unfold`, `row_idx`/`col_idx` are specified to set which modes are used
as the row/column.

```{r Unfold/Fold operations 1, echo=TRUE}
dmat1 <- unfold(darr1, row_idx=c(1,2), col_idx=3)
dmat1
```
...
There are some wrapper fuctions of `unfold` and `fold`.
...
dmat8 <- cs_unfold(darr1, m=1)
dmat8_to_darr1 <- cs_fold(dmat8, m=1, modes=dim(darr1))
identical(as.array(darr1), as.array(dmat8_to_darr1))

dmat9 <- cs_unfold(darr1, m=2)
dmat9_to_darr1 <- cs_fold(dmat9, m=2, modes=dim(darr1))
identical(as.array(darr1), as.array(dmat9_to_darr1))

dmat10 <- cs_unfold(darr1, m=3)
dmat10_to_darr1 <- cs_fold(dmat10, m=3, modes=dim(darr1))
...
dmat11 <- matvec(darr1)
dmat11_darr1 <- unmatvec(dmat11, modes=dim(darr1))
...
Inner product multiplies two tensors and collapsed to 0D tensor (norm).
On the hand, outer product is an operation that leaves all subscripts intact.

![Figure 4: Outer Product](Figure2_4.png)

```{r Outer Product, echo=TRUE}
outerProd(darr1[,,1], darr2[,,1])
```
...
prod_hl <- hadamard_list(list(darr1, darr2))
dim(prod_hl)
```
...
product and the dimensions of output tensor is ${IK \times JL}$.
...
prod_kron <- kronecker(darr1, darr2)
dim(prod_kron)
```
...
prod_kronl <- kronecker_list(list(darr1, darr2))
dim(prod_kronl)
```
...
prod_kr <- khatri_rao(darr1[,,1], darr2[,,1])
dim(prod_kr)
```
...
prod_krl <- khatri_rao_list(list(darr1[,,1], darr2[,,1]))
dim(prod_krl)
```
...
dim(modebind_list(list(darr1, darr2), m=1))
dim(modebind_list(list(darr1, darr2), m=2))
dim(modebind_list(list(darr1, darr2), m=3))
```
...
dim(rbind_list(list(darr1[,,1], darr2[,,1])))
```
...
dim(cbind_list(list(darr1[,,1], darr2[,,1])))
```
