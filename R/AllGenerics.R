...
# 	function(darr){
# 		standardGeneric("t")})
# setMethod("t", signature(darr="DelayedArray"),
# 	function(darr){
# 		.t(darr)})
# .t <- function(darr){
# 	if(.ndim(darr) != 3){
# 		stop("Tensor Transpose currently only implemented for 3d Tensors")
# 	}
# 	# e.g. 2 × 3 × 4
# 	modes <- dim(darr)
# 	new_arr <- darr[,,c(1L,modes[3]:2L), drop=FALSE]
# 	# e.g. 3 × 2 × 4
# 	.realize_and_return(aperm(new_arr, c(2,1,3)))
# }
...
# .reshapeIncNumbers2D <- function(mat, new_modes){
# 	# Setting
# 	block.size <- getAutoBlockSize()
# 	num_modes <- length(new_modes)
#     # Block Size Scheduling
#     mat_spacings <- .blockSizeScheduling5(dim(mat), 2, block.size)

#     # e.g. 3×40 => [3×8], [3×8], [3×8], [3×8], [3×8]
#     mat_grid <- RegularArrayGrid(
#         refdim=dim(mat),
#         spacings=mat_spacings)

#     # Block Size Scheduling
# 	# sink_spacings <- .blockSizeScheduling5(new_modes, num_modes, block.size)
# 	sink_spacings <- rep(1, length=length(new_modes))
# 	sink_spacings[seq_along(mat_spacings)] <- mat_spacings
#     # e.g. 3×4×2×5 => [3×4×2×1], [3×4×2×1], [3×4×2×1], [3×4×2×1], [3×4×2×1]
#     sink_grid <- RegularArrayGrid(
#         refdim=new_modes,
#         spacings=sink_spacings)
...
#     # Check
#     .checkLimit(sink_grid, block.size)
#     .checkLimit(mat_grid, block.size)
#     stopifnot(identical(length(sink_grid), length(mat_grid)))
#     # Block processing
# 	setAutoRealizationBackend("HDF5Array")
# 	sink <- AutoRealizationSink(new_modes)
# 	FUN <- function(sink_viewport, sink) {
