...
# # for .reshapeIncNumbers2D
# .blockSizeScheduling5 <- function(modes, num_modes, block.size){
# 	spacings <- rep(0, length=num_modes)
# 	for(i in seq(num_modes)){
# 		limit <- prod(modes[seq(i)])
# 		if(limit <= block.size){
# 			spacings[i] <- modes[i]
# 		}else{
# 			spacings[i] <- max(1,
# 				floor(block.size / prod(modes[seq(i-1)])))
# 		}
# 	}
# 	spacings
# }
