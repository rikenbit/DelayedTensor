...
In this vignette, we willl use CRAN `r CRANpkg("einsum")` package first.
...
It is extreamly fast because the internal calculation
...
By using multiple array or `r Biocpkg("DelayedArray")` object as input and
...
Outer product can also be implemented in `einsum`,
...
These are the same as what `modeSum` function does.
...
Some examples of combining Multiplication and Summation is shown below.
...
Inner product is an operation that eliminates all subscripts,
while outer product is an operation that leaves all subscripts intact.
...
Matrix Multiplication is considered as a contracted product.
...
Some examples of combining Multiplication and Permutation is shown below.
...
Some examples of combining Summation and Permutation is shown below.
...
Finally, we will show more complex example,
...
darr1 <- DelayedArray(array(runif(3*4), dim=c(3,4)))
darr2 <- DelayedArray(array(runif(5*6), dim=c(5,6)))

mykronecker <- function(darr1, darr2){
    stopifnot((length(dim(darr1)) == 2) && (length(dim(darr2)) == 2))
    # Outer Product
    tmpdarr <- einsum('ij,kl->iklj', darr1, darr2)

    # Reshape
    unfold(tmpdarr, row_idx=1:2, col_idx=3:4)
}

identical(as.array(kronecker(darr1, darr2)),
